...
import ApplyProcessHero from "@/components/ApplyProcess/ApplyProcessHero";
import WorkerCategories from "@/components/ApplyProcess/WorkerCategories";
import ApplySteps from "@/components/ApplyProcess/ApplySteps";
import ApplyCTA from "@/components/ApplyProcess/ApplyCTA";
...
async function getPageData() {
  ...
  try {
    ...
    return {
      header: result.data?.homePage[0]?.header || null,
      footer: result.data?.homePage[0]?.footer || null,
      navigations: result.data?.activeNavigations || [],
      categories: result.data?.applicationProcessPage[0]?.categories || [],
    };
  } catch (error) {
...
export default async function ApplyProcessPage() {
  try {
    const { header, footer, navigations, categories } = await getPageData();

    if (!header || !footer) {
      return (
        <main className="flex min-h-screen items-center justify-center">
          <div className="max-w-2xl p-8 text-center">
            <h1 className="text-2xl font-bold text-gray-800 mb-4">⚠️ 無法載入頁面資料</h1>
            <p className="text-gray-600">資料庫中沒有找到必要的頁面內容。請先在後台設定首頁資料。</p>
          </div>
        </main>
      );
    }
      ...
      <main className="flex min-h-screen items-center justify-center bg-gray-50 p-4">
        <div className="max-w-4xl w-full bg-white rounded-lg shadow-lg p-8">
          <div className="border-l-4 border-red-500 pl-4 mb-6">
            <h1 className="text-3xl font-bold text-red-600 mb-2">⚠️ 申請流程頁面載入失敗</h1>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            ...
            <ul className="list-disc list-inside space-y-2 text-gray-700">
              ...
              <li>檢查 <code className="bg-gray-100 px-2 py-1 rounded">.env</code> 檔案中的 <code className="bg-gray-100 px-2 py-1 rounded">NEXTAUTH_URL</code> 是否設定正確為 http://localhost:3001</li>
